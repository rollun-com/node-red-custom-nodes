...
let apiInstance = new FulfillmentApi.OrderApi();
...
apiInstance.getOrder(orderId, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
...
<a name="getOrders"></a>
# **getOrders**
> OrderSearchPagedCollection getOrders(opts)
...
# **issueRefund**
> Refund issueRefund(orderId, opts)
