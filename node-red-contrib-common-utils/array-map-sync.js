module.exports = function (RED) {
  ...
  RED.nodes.registerType("array-map-start-sync", ArrayMapStart);
  ...
  function ArrayMapEnd(config) {
    ...
    node.on('input', function (msg) {
      ...
      !timeout && timeoutTime > 0 && (timeout = setTimeout(() => {
        ...
        clearResult(msg);
        msg.payload = {error: `Did not receive all items from array-map-start-sync after ${timeoutTime}ms`};
        node.send(msg);
        timeouted = true;
      }, timeoutTime));
  ...
  RED.nodes.registerType("array-map-end-sync", ArrayMapEnd);
